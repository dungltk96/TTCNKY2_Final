...
namespace ShopApp
{
    public partial class frmProducts : Form
    {
        ...
        private void frmProducts_Load(object sender, EventArgs e)
        {
            ...
            LoadDataGridView();
            SqlCommand cmd = Functions.RunProcedure("GetCategories"); // Lấy danh sách danh mục
            cmd.ExecuteNonQuery();
        ...
        private void LoadDataGridView()
        {
            ...
            // Thêm cột Origin_price nếu chưa có
            if (!dgvProducts.Columns.Contains("Origin_price"))
            {
                DataGridViewTextBoxColumn originPriceColumn = new DataGridViewTextBoxColumn();
                originPriceColumn.DataPropertyName = "Origin_price";
                originPriceColumn.Name = "Origin_price";
                originPriceColumn.HeaderText = "Origin Price";
                dgvProducts.Columns.Add(originPriceColumn);
            }
        ...
        // format tiền tệ 10/06/24
        private void dgvProducts_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dgvProducts.Columns[e.ColumnIndex].Name == "Origin_price" && e.Value != null)
            {
                e.Value = Functions.FormatMoney(e.Value.ToString());
                e.FormattingApplied = true;
            }

            if (dgvProducts.Columns[e.ColumnIndex].Name == "Sale_price" && e.Value != null)
            {
                e.Value = Functions.FormatMoney(e.Value.ToString());
                e.FormattingApplied = true;
            }
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
           ...
           /* if (cbStatus.SelectedItem.Equals("Hết hàng"))
            ...
            int origin = Int32.Parse(txtOriginPrice.Text); // giá gốc
            int salep = Int32.Parse(txtSalePrice.Text); // giá khuyến mãi

            // Tính số tiền được giảm
            float discountAmount = origin - salep;

            // Tính phần trăm giảm giá
            double discountPercentage = ((double)discountAmount / origin) * 100;
            ...
            cmd.Parameters.Add(new SqlParameter("@Discount_percent", discountPercentage));
